...
public abstract class Partitioner implements Writable {
	 ...
	 *             if points are empty
	 */
	public abstract void createFromPoints(Rectangle mbr, Point[] points, int capacity) throws IllegalArgumentException;
	 ...
	 * than one partitions, the partitioner returns only one of them according
	 * to its own criteria. If it does not overlap any partition, it returns -1
	...
	/**
	 * Returns the detail of a partition given its index starting at zero and
	 * ending at partitionCount() - 1
	 * 
	 * @param index
	 * @return
	 */
	...
	public static void setFilePartitioner(Configuration conf, FilePartitioner partitioner) throws IOException {
	...
	public static Partitioner getFilePartitioner(Configuration conf) throws IOException {
		FilePartitioner partitioner = new FilePartitioner();
		String partitionerFile = conf.get(PartitionerValue);
		System.out.println("partitioner file path = " + partitionerFile);
		...
		if (partitionerFile != null) {
			...
			for (Path cacheFile : cacheFiles) {
				if (cacheFile.getName().contains(partitionerFile)) {
					...
					LineReader in = new LineReader(inFs.open(cacheFile));
					while (in.readLine(tempLine) > 0) {
						System.out.println("temp line = " + tempLine.toString());
						Partition tempPartition = new Partition();
						tempPartition.fromText(tempLine);
						partitioner.partitions.add(tempPartition);
					}
					in.close();
				}
			}
