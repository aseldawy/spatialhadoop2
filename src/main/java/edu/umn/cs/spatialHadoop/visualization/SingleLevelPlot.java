...
public class SingleLevelPlot {
	...
	/**
	 * Visualizes a dataset using the existing partitioning of a file. The
	 * mapper creates a partial canvas for each partition while the reducer
	 * merges the partial canvases together into the final canvas. The final
	 * canvas is then written to the output.
	 * 
	 * @author Ahmed Eldawy
	 *
	 */
	public static class NoPartitionPlotMap<S extends Shape>
		...
		protected void map(Rectangle partitionMBR, Iterable<S> shapes, Context context)
			...
			// If input is not spatially partitioned, the MBR is taken from
			// input
			...
			// because it will cause round-off errors between adjacent
			// partitions
			...
			// If we set the output value to one constant, all intermediate
			// layers
			// will be merged in one machine. Alternatively, We can set it to
			// several values
	...
	public static class NoPartitionPlotCombine<S extends Shape>
		...
		@Override
		protected void reduce(IntWritable key, Iterable<Canvas> intermediateLayers, Context context)
				throws IOException, InterruptedException {
	...
	public static class NoPartitionPlotReduce<S extends Shape>
		...
		@Override
		protected void reduce(IntWritable dummy, Iterable<Canvas> intermediateLayers, Context context)
				throws IOException, InterruptedException {
			// TODO Auto-generated method stub
			Canvas finalLayer = plotter.createCanvas(imageWidth, imageHeight, inputMBR);
	...
	/**
	 * Generates a single level using a MapReduce job and returns the created
	 * job.
	 * 
	 * @param inFiles
	 * @param outFile
	...
	public static Job plotMapReduce(Path[] inFiles, Path outFile, Class<? extends Plotter> plotterClass,
		...
		if (partition.equalsIgnoreCase("none")) {
			...
			if (partition.equals("pixel")) {
				// Special case for pixel level partitioning as it depends on
				// the
	...
	public static void plotLocal(Path[] inFiles, Path outFile, final Class<? extends Plotter> plotterClass,
		...
		String mbr = params.get(InputMBR);
		// System.out.println("input mbr = " + mbr);
//		final Rectangle inputMBR = params.get(InputMBR) != null
//				? params.getShape("mbr", new Rectangle(-180.0, -90.0, 180.0, 83.0)).getMBR()
//				: FileMBR.fileMBR(inFiles, mbrParams);
		final Rectangle inputMBR = new Rectangle(-180.0, -90.0, 180.0, 83.0);
		...
		if (params.getBoolean("keepratio", true)) {
			// Adjust width and height to maintain aspect ratio and store the
			// adjusted
		...
		// Store width and height in final variables to make them accessible in
		// parallel
		...
		for (Path inFile : inFiles) {
			...
			if (!OperationsParams.isWildcard(inFile) && inFs.exists(inFile) && !inFs.isDirectory(inFile)) {
				if (SpatialSite.NonHiddenFileFilter.accept(inFile)) {
					// Use the normal input format splitter to add this
					// non-hidden file
					...
					// automatically
					// skipped by FileInputFormat. We need to plot a hidden file
					// for the case
					// of plotting partition boundaries of a spatial index
					splits.add(new FileSplit(inFile, 0, inFs.getFileStatus(inFile).getLen(), new String[0]));
				}
			} else {
				// Use the normal input format splitter to add this non-hidden
				// file
				Job job = Job.getInstance(params);
				SpatialInputFormat3.addInputPath(job, inFile);
				splits.addAll(inputFormat.getSplits(job));
		...
		List<Canvas> partialCanvases = Parallel.forEach(fsplits.length, new RunnableRange<Canvas>() {
			...
			public Canvas run(int i1, int i2) {
				...
				// Create the partial layer that will contain the plot of the
				// assigned partitions
	 ...
	 * @throws ClassNotFoundException
	 */
	public static Job plot(Path[] inFiles, Path outFile, final Class<? extends Plotter> plotterClass,
