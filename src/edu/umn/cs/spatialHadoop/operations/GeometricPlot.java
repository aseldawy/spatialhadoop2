 ...
 * 
 * This operation has three implementations. 1) Fast Plot. This operation uses
 * data partitioning to draw an image for each split and then overlay images on
 * top of each other to produce final result. To use this method, you need to
 * set the parameter "partition:data" 2) Partitioned Plot. This implementation
 * uses space partitioning to partition the data and then plots an image for
 * each partition. After that, it stitches these images together to produce the
 * final picture. To use this method, set the parameter "partition:space" 3) If
 * the data is already spatially partitioned using an R+-tree or Grid file, and
 * "partition:space" is provided, this algorithm uses the existing partitioning
 * to draw an image for each partition and then combine the results back in one
 * machine by stitching them.
 * 
 * @author Ahmed Eldawy
 *
 */
public class GeometricPlot {
	 ...
	 * partition is not partitioned (a heap file), then the given shape is
	 * output to all overlapping partitions.
	 * 
	 * @author Ahmed Eldawy
	 *
	 */
	public static class GridPartitionMap extends MapReduceBase implements
			Mapper<Rectangle, Object, IntWritable, Shape> {
		...
		public void configure(JobConf job) {
			super.configure(job);
			this.partitionGrid = (GridInfo) OperationsParams.getShape(job,
					PartitionGrid);
			...
			if (this.adaptiveSample) {
				...
				this.adaptiveSampleRatio = job.getFloat(AdaptiveSampleRatio,
						0.01f);
			}
		}
		...
		public void map(Rectangle cell, Object value,
			...
			if (value instanceof Shape) {
				...
				if (shapeMbr == null)
					return;
				// Check if this shape can be skipped using the gradual fade
				// option
				if (gradualFade && !(shape instanceof Point)) {
					double areaInPixels = (shapeMbr.getWidth() + shapeMbr
							.getHeight()) * scale;
					if (areaInPixels < 1.0
							&& Math.round(areaInPixels * 255) < 1.0) {
						// This shape can be safely skipped as it is too small
						// to be plotted
						return;
					}
				}
				...
				if (queryRange != null && !shapeMbr.isIntersected(queryRange))
					return;
				java.awt.Rectangle overlappingCells = partitionGrid
				...
				while (((ShapeIterator) value).hasNext()) {
					...
					if (gradualFade && !(shape instanceof Point)) {
						...
						if (areaInPixels < 1.0
								&& Math.round(areaInPixels * 255) < 1.0) {
							// This shape can be safely skipped as it is too
							// small to be plotted
							continue;
						}
					}
					...
					// Skip shapes outside query range if query range is set
					if (queryRange != null
							&& !shapeMbr.isIntersected(queryRange))
						continue;
					java.awt.Rectangle overlappingCells = partitionGrid
							.getOverlappingCells(shapeMbr);
					for (int i = 0; i < overlappingCells.width; i++) {
	 ...
	 *
	 */
	public static class GridPartitionReduce extends MapReduceBase implements
			Reducer<IntWritable, Shape, Rectangle, ImageWritable> {
		...
		public void reduce(IntWritable cellNumber, Iterator<Shape> values,
				OutputCollector<Rectangle, ImageWritable> output,
				Reporter reporter) throws IOException {
			try {
				CellInfo cellInfo = partitionGrid.getCell(cellNumber.get());
				// Initialize the image
				int image_x1 = (int) Math.floor((cellInfo.x1 - fileMbr.x1)
						* imageWidth / fileMbr.getWidth());
				int image_y1 = (int) Math.floor((cellInfo.y1 - fileMbr.y1)
						* imageHeight / fileMbr.getHeight());
				int image_x2 = (int) Math.ceil((cellInfo.x2 - fileMbr.x1)
						* imageWidth / fileMbr.getWidth());
				int image_y2 = (int) Math.ceil((cellInfo.y2 - fileMbr.y1)
						* imageHeight / fileMbr.getHeight());
				int tile_width = image_x2 - image_x1;
				int tile_height = image_y2 - image_y1;

				BufferedImage image = new BufferedImage(tile_width,
						tile_height, BufferedImage.TYPE_INT_ARGB);
					...
					if (fade) {
						...
						if (areaInPixels > 1.0) {
							...
							if (alpha == 0) {
								...
								graphics.setColor(new Color(((int) alpha << 24)
										| strokeColor, true));
							}
						}
					}
	 ...
	 *
	 */
	public static class SkewedPartitionReduce extends MapReduceBase implements
			Reducer<IntWritable, Shape, Rectangle, ImageWritable> {
		...
		public void configure(JobConf job) {
			...
			this.bufferSize = job.getInt("buffer", 3);
		...
		public void reduce(IntWritable cellNumber, Iterator<Shape> values,
			...
			try {
				...
				int iCell = 0;
				while (iCell < cells.length
						&& cells[iCell].cellId != cellNumber.get())
					iCell++;
				if (iCell >= cells.length)
					throw new RuntimeException("Cannot find cell: "
							+ cellNumber);
				...
				while (values.hasNext()) {
					...
					if (clean) {
						...
						ogcjtsshape.geom = DouglasPeuckerSimplifier.simplify(
								ogcjtsshape.geom, tolerance);
						...
						if (i < shapeList.length - 1) {
							...
							if (shapeList.length > 1) {
								...
								GeometryCollection geo_collection = geometryFactory
										.createGeometryCollection(shapeList);
						...
						s.draw(graphics, fileMBR, imageWidth, imageHeight,
								scale2);
					}
				}
				if (clean) {
					...
					GeometryFactory geometryFactory = new GeometryFactory();
					GeometryCollection geo_collection = geometryFactory
							.createGeometryCollection(thisSplitList);
					resultGeometry = geo_collection.buffer(0);
					OGCJTSShape combinedShape = new OGCJTSShape(resultGeometry);
					combinedShape.draw(graphics, fileMBR, imageWidth,
							imageHeight, scale2);
				}
				graphics.dispose();
	 ...
	 *
	 */
	public static class AlreadyPartitionedMap extends MapReduceBase implements
			Mapper<Rectangle, ShapeIterator, Rectangle, ImageWritable> {
		...
		public void map(Rectangle key, ShapeIterator value,
				...
				Reporter reporter) throws IOException {
			// Initialize the image
			int image_x1 = (int) Math.floor((key.x1 - fileMBR.x1) * imageWidth
					/ fileMBR.getWidth());
			int image_y1 = (int) Math.floor((key.y1 - fileMBR.y1) * imageHeight
					/ fileMBR.getHeight());
			int image_x2 = (int) Math.ceil((key.x2 - fileMBR.x1) * imageWidth
					/ fileMBR.getWidth());
			int image_y2 = (int) Math.ceil((key.y2 - fileMBR.y1) * imageHeight
					/ fileMBR.getHeight());
			int tile_width = image_x2 - image_x1;
			int tile_height = image_y2 - image_y1;
			...
			if (clean) {
				...
				resultGeometry = geo_collection.buffer(0);
				OGCJTSShape combinedShape = new OGCJTSShape(resultGeometry);
				combinedShape.draw(graphics, fileMBR, imageWidth, imageHeight,
						scale2);
			}
			long endTime = System.currentTimeMillis();
			
			LOG.info("TIME TAKEN: " + (endTime - startTime) + " milis");
			graphics.dispose();
			sharedValue.setImage(image);
	...
	public static class PlotOutputCommitter extends FileOutputCommitter {
		...
		public void commitJob(JobContext context) throws IOException {
			...
			Color bgColor = OperationsParams.getColor(job, "bgcolor",
					new Color(0, 0, 0, 0));
	...
	public static class DataPartitionMap extends MapReduceBase implements
		...
		public void configure(JobConf job) {
			...
			super.configure(job);
			this.queryRange = OperationsParams.getShape(job, "rect");
			this.drawMbr = queryRange != null ? queryRange.getMBR()
					: ImageOutputFormat.getFileMBR(job);
			this.imageWidth = job.getInt("width", 1000);
			this.imageHeight = job.getInt("height", 1000);
			this.strokeColor = job.getInt("color", 0);
			this.fade = job.getBoolean("fade", false);
			...
			while (value.hasNext()) {
				...
				if (queryRange == null || queryRange.isIntersected(shape)) {
					if (fade) {
						Rectangle shapeMBR = shape.getMBR();
						if (shapeMBR != null) {
							// shapeArea represents how many pixels are covered
							// by shapeMBR
							double shapeArea = (shapeMBR.getWidth() + shapeMBR
					...
					shape.draw(graphics, drawMbr, imageWidth, imageHeight,
							scale2);
				}
			}
	...
	public static class DataPartitionReduce extends MapReduceBase implements
			Reducer<Rectangle, ImageWritable, Rectangle, ImageWritable> {
	...
	public static RunningJob plotMapReduce(Path inFile, Path outFile,
		...
		if (valueRangeStr == null) {
			...
			valueRange = new MinMax(Integer.parseInt(parts[0]),
		...
		if (hdfDataset != null) {
			...
			job.set("valuerange", valueRange.minValue + ".."
					+ valueRange.maxValue);
			...
			fileMBR = plotRange != null ? plotRange.getMBR() : FileMBR.fileMBR(
					inFile, mbrArgs);
		}
		...
		if (keepAspectRatio) {
			...
			if (fileMBR.getWidth() / fileMBR.getHeight() > (double) width
					/ height) {
				// Fix width and change height
				height = (int) (fileMBR.getHeight() * width / fileMBR
						.getWidth());
				// Make divisible by two for compatibility with ffmpeg
				height &= 0xfffffffe;
				job.setInt("height", height);
			} else {
				width = (int) (fileMBR.getWidth() * height / fileMBR
						.getHeight());
		...
		GlobalIndex<Partition> gindex = SpatialSite
				.getGlobalIndex(inFs, inFile);
		if (hdfDataset != null)
			partition = "grid";
		...
		if (partition.equals("space") || partition.equals("grid")) {
			if (gindex != null && gindex.isReplicated()) {
				...
				GridInfo partitionGrid = new GridInfo(fileMBR.x1, fileMBR.y1,
						fileMBR.x2, fileMBR.y2);
				partitionGrid.calculateCellDimensions((int) Math.max(1,
				...
				CellInfo[] cellInfos = Repartition.packInRectangles(inFile,
						outFile, params, fileMBR);
			...
			throw new RuntimeException("Unknown partition scheme '"
					+ job.get("partition") + "'");
		}
		...
		// Set output committer which will stitch images together after all
		// reducers
		// finish
	...
	private static <S extends Shape> void plotLocal(Path inFile, Path outFile,
			OperationsParams params) throws IOException {
		String hdfDataset = (String) params.get("dataset");
		Shape shape = hdfDataset != null ? new NASARectangle() : (Shape) params
				.getShape("shape", null);
		Shape plotRange = params.getShape("rect", null);
		...
		if (inFStatus != null && !inFStatus.isDir()) {
			...
			splits = new InputSplit[] { new FileSplit(inFile, 0,
					inFStatus.getLen(), new String[0]) };
		} else {
		...
		boolean adaptiveSample = shape instanceof Point
				&& params.getBoolean("sample", false);
		...
		boolean gradualFade = !(shape instanceof Point)
		...
		if (hdfDataset != null) {
			...
			NASAPoint.gradientType = params.getGradientType("gradient",
					NASAPoint.GradientType.GT_HUE);
		}
		...
		for (InputSplit split : splits) {
			if (hdfDataset != null) {
				...
				RecordReader<NASADataset, NASAShape> reader = new HDFRecordReader(
						params, (FileSplit) split, hdfDataset, true);
				...
				RecordReader<Rectangle, ShapeIterator> reader = new ShapeIterRecordReader(
						params, (FileSplit) split);
				...
				while (reader.next(cell, value)) {
					for (Shape s : value) {
						...
						ogcjtsshape.geom = DouglasPeuckerSimplifier.simplify(
								ogcjtsshape.geom, toleranceDistance);
						BufferOp bufOp = new BufferOp(ogcjtsshape.geom);
						bufOp.setEndCapStyle(BufferOp.CAP_SQUARE);
						...
						if (i < maxArraySize - 1) {
							...
							if (shapeList.length > 1) {
								...
								GeometryCollection geo_collection = geometryFactory
										.createGeometryCollection(shapeList);
								LOG.info("Combining " + maxArraySize
										+ " geometries.");
								resultGeometry = geo_collection.buffer(0);
								shapeList = new Geometry[maxArraySize];
								shapeList[0] = resultGeometry;
				...
				GeometryCollection geo_collection = geometryFactory
						.createGeometryCollection(thisSplitList);
				LOG.info("Combining the geometry for split " + currentSplit
						+ " out of " + splits.length);
				resultGeometry = geo_collection.buffer(0);
				reader.close();
	...
	public static RunningJob plot(Path inFile, Path outFile,
			OperationsParams params) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static void drawScale(Path output, MinMax valueRange, int width,
		...
		int step = (valueRange.maxValue - valueRange.minValue) * fontSize * 10
				/ height;
		...
		for (int value = min_value; value <= max_value; value += step) {
			int y = fontSize + (height - fontSize) - value
					* (height - fontSize)
	...
	public static BufferedImage combineImages(Configuration conf, Path[] files,
		...
		for (Path file : files) {
			...
			if (fs.getFileStatus(file).isDir()) {
				...
				Rectangle mbr = FileMBR.fileMBR(file,
						new OperationsParams(conf));
				...
				if (!fs.exists(imagePath))
					throw new RuntimeException("Image " + imagePath
							+ " not ready");
				...
				if (includeBoundaries) {
					...
					if (fs.exists(imagePath)) {
						...
						graphics.drawImage(image, (int) mbr.x1, (int) mbr.y1,
								(int) mbr.getWidth(), (int) mbr.getHeight(),
								null);
						graphics.dispose();
					}
				}
	...
	private static void printUsage() {
		...
		System.out
				.println("partition:<data|space> - whether to use data partitioning (default) or space partitioning");
